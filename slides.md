...
const TYPES = ['driver', 'passenger', 'pedestrian', 'cyclist']
const TYPE_LABELS = { driver: 'Driver', passenger: 'Passenger', pedestrian: 'Ped', cyclist: 'Cyclist' }
const COLORS = { driver: '#a94c9a', passenger: '#f08030', pedestrian: '#d85a6a', cyclist: '#7c5295' }
const ROWS = [
...
function selectAll() { selected.value = [...TYPES] }
function clickType(t) {
  if (allSelected()) { selected.value = [t]; return }
  const s = new Set(selected.value)
  s.has(t) ? s.delete(t) : s.add(t)
  if (s.size === 0 || s.size === TYPES.length) selectAll()
...
<div class="rows">
  <div class="urow" v-for="row in ROWS" :key="row.key">
    <div class="meta">
      <span class="n">{{ rowTotal(row.key).toLocaleString() }}</span>
...
.slidev-layout.unitgrid-exp {
  ...
  .meta .n { font-size: 1.5rem; font-weight: 700; color: var(--hccs-accent); font-variant-numeric: tabular-nums; }
  .meta .lab { font-size: 0.95rem; opacity: 0.9; }
  .box {
    height: 100%;
    min-height: 0;
